...
void
execute(Text *t, uint32_t aq0, uint32_t aq1, int external, Text *argt)
{
	uint32_t q0, q1;
	Rune *r, *s;
	char *b, *a, *aa;
...
char*
printarg(Text *argt, uint32_t q0, uint32_t q1)
{
	char *buf;
...
runproc(void *argvp)
{
	...
	s = argv[1];
	rdir = argv[2];
	ndir = (uintptr_t)argv[3];
	newns = (uintptr_t)argv[4];
	argaddr = argv[5];
	arg = argv[6];
	c = argv[7];
	cpid = argv[8];
	iseditcmd = (uintptr_t)argv[9];
	free(argv);
...
Hard:
	...
	procexecl(cpid, "/cmd/rc", "rc", "-c", t, nil);
