...
struct Undo
{
	...
	uint32_t		p0;		/* location of change (unused in f) */
	uint32_t		n;		/* # runes in string or file name */
};
...
void
fileinsert(File *f, uint32_t p0, Rune *s, uint32_t ns)
{
	if(p0 > f->nc)
...
void
fileuninsert(File *f, Buffer *delta, uint32_t p0, uint32_t ns)
{
	Undo u;
...
void
filedelete(File *f, uint32_t p0, uint32_t p1)
{
...
void
fileundelete(File *f, Buffer *delta, uint32_t p0, uint32_t p1)
{
	Undo u;
	Rune *buf;
	uint32_t i, n;
...
uint32_t
fileload(File *f, uint32_t p0, int fd, int *nulls)
{
	if(f->seq > 0)
...
void
fileundo(File *f, int isundo, uint32_t *q0p, uint32_t *q1p)
{
	Undo u;
	Rune *buf;
	uint32_t i, j, n, up;
