...
winsettag1(Window *w)
{
	...
	i += w->body.file->nname;
	runemove(new+i, (Rune*)L" Del Snarf", 10);
	i += 10;
	if(w->filemenu){
		if(w->body.file->delta.nc>0 || w->body.ncache){
			runemove(new+i, (Rune*)L" Undo", 5);
			i += 5;
		}
		if(w->body.file->epsilon.nc > 0){
			runemove(new+i, (Rune*)L" Redo", 5);
			i += 5;
		}
		dirty = w->body.file->nname && (w->body.ncache || w->body.file->seq!=w->putseq);
		if(!w->isdir && dirty){
			runemove(new+i, (Rune*)L" Put", 4);
			i += 4;
		}
	}
	if(w->isdir){
		runemove(new+i, (Rune*)L" Get", 4);
		i += 4;
	}
	runemove(new+i, (Rune*)L" |", 2);
	i += 2;
	r = runestrchr(old, '|');
	if(r)
		k = r-old+1;
