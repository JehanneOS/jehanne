...
static int
rootdirgen(int n, Dir *dir, void* _)
{
	if(n > 0)
		return -1;
...
static int
readlist(int off, int (*gen)(int, char*, uint32_t, Fsstate*), Req *r, Fsstate *fss)
{
	char *a, *ea;
	int n;
...
static int
keylist(int i, char *a, uint32_t n, Fsstate *fss)
{
	int wb;
	Keyinfo ki;
	Key *k;
...
static int
protolist(int i, char *a, uint32_t n, Fsstate *fss)
{
	USED(fss);
