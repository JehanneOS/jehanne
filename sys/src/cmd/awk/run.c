...
Cell *program(Node **a, int _)	/* execute an awk program */
...
Cell *call(Node **a, int _)	/* function call.  very kludgy and fragile */
{
...
Cell *getnf(Node **a, int _)	/* get NF */
{
	if (donefld == 0)
		fldbld();
	return (Cell *) a[0];
...
Cell *array(Node **a, int _)	/* a[0] is symtab, a[1] is list of subscripts */
{
	Cell *x, *y, *z;
	char *s;
	Node *np;
...
Cell *awkdelete(Node **a, int _)	/* a[0] is symtab, a[1] is list of subscripts */
{
	Cell *x, *y;
	Node *np;
	char *s;
...
Cell *intest(Node **a, int _)	/* a[0] is index (list), a[1] is symtab */
{
	Cell *x, *ap, *k;
	Node *p;
	char *buf;
...
Cell *indirect(Node **a, int _)	/* $( a[0] ) */
{
	Cell *x;
	int m;
	char *s;
...
Cell *substr(Node **a, int _)		/* substr(a[0], a[1], a[2]) */
{
	int k, m, n;
	Rune r;
	char *s, *p;
...
Cell *sindex(Node **a, int _)		/* index(a[0], a[1]) */
{
	Cell *x, *y, *z;
	char *s1, *s2, *p1, *p2, *q;
	Awkfloat v = 0.0;
...
int format(char **pbuf, int *pbufsize, char *s, Node *a)	/* printf-like conversions */
{
	...
	while (*s) {
		...
		*t = '\0';
		...
		case 2:	sprint(p, fmt, (int32_t) getfval(x)); break;
...
Cell *awksprintf(Node **a, int _)		/* sprint(a[0]) */
{
	Cell *x;
	Node *y;
	char *buf;
...
Cell *awkprintf(Node **a, int _)		/* printf */
...
Cell *cat(Node **a, int _)	/* a[0] cat a[1] */
{
	Cell *x, *y, *z;
	int n1, n2;
	char *s;
...
Cell *pastat(Node **a, int _)	/* a[0] { a[1] } */
{
	Cell *x;
...
Cell *dopa2(Node **a, int _)	/* a[0], a[1] { a[2] } */
{
	Cell *x;
	int pair;
...
Cell *split(Node **a, int _)	/* split(a[0], a[1], a[2]); a[3] is type */
{
	Cell *x = 0, *y, *ap;
	char *s, *t, *fs = 0;
	char temp, num[50];
...
Cell *condexpr(Node **a, int _)	/* a[0] ? a[1] : a[2] */
{
	Cell *x;
...
Cell *ifstat(Node **a, int _)	/* if (a[0]) a[1]; else a[2] */
{
	Cell *x;
...
Cell *whilestat(Node **a, int _)	/* while (a[0]) a[1] */
{
	Cell *x;
...
Cell *dostat(Node **a, int _)	/* do a[0]; while(a[1]) */
{
	Cell *x;
...
Cell *forstat(Node **a, int _)	/* for (a[0]; a[1]; a[2]) a[3] */
{
	Cell *x;
...
Cell *instat(Node **a, int _)	/* for (a[0] in a[1]) a[2] */
{
...
Cell *bltin(Node **a, int _)	/* builtin functions. a[0] is type, a[1] is arg list */
{
	Cell *x, *y;
	Awkfloat u;
	int t;
...
Cell *printstat(Node **a, int _)	/* print a[0] */
{
	int r;
	Node *x;
	Cell *y;
...
Cell *sub(Node **a, int _)	/* substitute command */
{
	char *sptr, *pb, *q;
	Cell *x, *y, *result;
	char *t, *buf;
...
Cell *gsub(Node **a, int _)	/* global substitute */
{
	Cell *x, *y;
