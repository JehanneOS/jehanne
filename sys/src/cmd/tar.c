...
/* read big-endian binary integers; args must be (uint8_t *) */
...
static void
putbe(uint8_t *dest, uint64_t vl, int size)
{
...
hdrsize(Hdr *hp)
{
	...
	if((uint8_t)hp->size[0] == Binnegsz) {
		fprint(2, "%s: %s: negative length, which is insane\n",
			argv0, name(hp));
		return 0;
	} else if((uint8_t)hp->size[0] == Binsize) {
		p = (uint8_t *)hp->size + sizeof hp->size - 1 -
...
mkhdr(Hdr *hp, Dir *dir, char *file)
{
	...
	if (dir->length >= (Off)1<<32) {
		...
		putbe((uint8_t *)hp->size+1, dir->length, sizeof hp->size - 2);
