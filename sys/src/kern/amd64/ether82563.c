...
struct Ctlr {
	...
	uint32_t	rdh;			/* receive descriptor head */
	uint32_t	rdt;			/* receive descriptor tail */
...
i82563tproc(void *v)
{
	...
	for(;;){
		...
		bp = qbread(edev->netif.oq, 100000);
		td = &ctlr->tdba[tdt];
...
static uint32_t phywrite0(Ctlr*, int, int, uint16_t);

static uint32_t
setpage(Ctlr *c, uint32_t phyno, uint32_t p, uint32_t r)
{
	uint32_t pr;
...
static uint32_t
phywrite(Ctlr *c, uint32_t phyno, uint32_t reg, uint16_t v)
{
	if(setpage(c, phyno, reg>>8, reg & 0xff) == ~0)
...
static void
phyerrata(Ether *e, Ctlr *c, uint32_t phyno)
{
	if(e->netif.mbps == 0)
		if(c->phyerrata == 0){
			c->phyerrata++;
...
{
	uint32_t i, rx;
	uint32_t tx __attribute__ ((unused));
	Ctlr *c;
	Ether *e;
...
static void
i82563interrupt(Ureg* _1, void *arg)
{
	Ctlr *ctlr;
	Ether *edev;
	uint32_t icr, im;
