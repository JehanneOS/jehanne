...
static long
igbeifstat(Ether* edev, void* a, long n, uint32_t offset)
{
	Ctlr *ctlr;
	char *p, *s;
	int i, l, r;
...
igbetxinit(Ctlr* ctlr)
{
	...
	csr32w(ctlr, Tdbal, PCIWADDR32(ctlr->tdba));
	csr32w(ctlr, Tdbah, 0);
...
igberxinit(Ctlr* ctlr)
{
	...
	csr32w(ctlr, Rdbal, PCIWADDR32(ctlr->rdba));
	csr32w(ctlr, Rdbah, 0);
