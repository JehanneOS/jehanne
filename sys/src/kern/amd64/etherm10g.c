...
enum {
	...
	CGcoaloff,	/* in microseconds */
	CSstatsrate,	/* in microseconds */
	CSstatsdma,
...
typedef struct {
	...
	Block	**bring;
//	uint8_t	*wcfifo;	/* what the heck is a w/c fifo? */
	int	size;		/* of buffers in the z8's memory */
	uint32_t	segsz;
	uint32_t	n;		/* rxslots */
	uint32_t	m;		/* mask; rxslots must be a power of two */
	uint32_t	i;		/* number of segments (not frames) queued */
	uint32_t	cnt;		/* number of segments sent by the card */
...
struct Bpool {
	Lock	l;
	Block	*head;
	uint32_t	size;		/* buffer size of each block */
	uint32_t	n;		/* n free buffers */
	uint32_t	cnt;
};
...
//	uint8_t	*wcfifo;	/* cmd submission fifo */
	uint32_t	m;
	uint32_t	n;		/* rxslots */
	uint32_t	i;
	uint32_t	cnt;		/* number of buffers allocated (lifetime) */
	uint32_t	allocfail;
} Rx;
...
static int
nsegments(Block *b, uint32_t segsz)
{
	uintmem bus, end, slen, len;
	int i;

	bus = PCIWADDR64(b->rp);
	i = 0;
	for(len = BLEN(b); len; len -= slen){
		end = bus + segsz & ~(uintmem)(segsz-1);
		slen = end - bus;
		if(slen > len)
			slen = len;
		bus += slen;
...
{
	uint16_t slen;
	uint32_t i, cnt, rdma, nseg, count, len;
	uint64_t bus, end, segsz;
	uint8_t flags;
	Block *b;
	Ctlr *c;
	Send *s, *s0, *s0m8;
...
static long
m10gifstat(Ether *e, void *v, long n, uint32_t off)
{
	int l, lim;
	char *p;
	Ctlr *c;
