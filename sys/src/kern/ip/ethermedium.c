...
etherread4(void *a)
{
	...
	for(;;){
		...
		ifc->in++;
		if(ifc->lifc == nil || BLEN(bp) <= ifc->medium->hsize)
			freeb(bp);
		else {
...
static void
etheraddmulti(Ipifc *ifc, uint8_t *a, uint8_t * _1)
{
	uint8_t mac[6];
	char buf[64];
	Etherrock *er = ifc->arg;
...
static void
etherremmulti(Ipifc *ifc, uint8_t *a, uint8_t * _1)
{
	uint8_t mac[6];
	char buf[64];
	Etherrock *er = ifc->arg;
...
recvarp(Ipifc *ifc)
{
	...
	static uint8_t eprinted[4], nullspa[4];
	Etherrock *er = ifc->arg;
