...
cclose(Chan *c)
{
	if(c == nil || c->r.ref < 1 || c->flag&CFREE)
...
ccloseq(Chan *c)
{
	...
	if(!wakeup(&clunkq.r)){
		...
		if(seq < 10)
			kproc("closeproc", closeproc, (void*)(uintptr_t)seq);
		else
			adec(&clunkq.active);
	}
}
...
int
eqchanddq(Chan *c, int dc, uint32_t devno, Qid qid, int skipvers)
{
...
cmount(Chan **newp, Chan *old, int flag, char *spec)
{
	...
	if(order != MREPL && (QTDIR & (old->qid.type^(*newp)->qid.type)))
		error(Emount);
...
int
findmount(Chan **cp, Mhead **mp, int dc, uint32_t devno, Qid qid)
{
	Pgrp *pg;
	Mhead *mh;
...
namec(char *aname, int amode, long omode, long perm)
{
	...
	if(amode == Aopen && (omode&7) == OEXEC && (c->qid.type & QTDIR))
