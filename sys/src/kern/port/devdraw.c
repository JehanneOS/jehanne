...
static int
drawgen(Chan *c, char* _, Dirtab* __, int ___, int s, Dir *dp)
{
	int t;
	Qid q;
	uint32_t path;
...
static
void
drawrefresh(Memimage* _, Rectangle r, void *v)
{
	Refx *x;
	DImage *d;
	Client *c;
...
makescreenimage(void)
{
	...
	if(sdraw.softscreen == 0xa110c){
		...
		md = *((Memdata**)(data - sizeof(uint32_t) - sizeof(Memdata*)));
...
static Chan*
drawattach(Chan *c, Chan *ac, char *spec, int flags)
{
	dlock();
	if(!initscreenimage()){
		dunlock();
...
static long
drawstat(Chan *c, uint8_t *db, long n)
{
...
long
drawread(Chan *c, void *a, long n, int64_t off)
{
	int index, m;
...
static long
drawwrite(Chan *c, void *a, long n, int64_t _)
{
