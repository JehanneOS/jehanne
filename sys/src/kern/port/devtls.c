...
struct Secret
{
	...
	DigestState	*(*mac)(uint8_t*, uint32_t, uint8_t*, uint32_t, uint8_t*, DigestState*);
...
struct TlsRec
{
	...
	void		(*packMac)(Secret*, uint8_t*, uint8_t*, uint8_t*, uint8_t*, int, uint8_t*);
...
static DigestState*sslmac_md5(uint8_t *p, uint32_t len, uint8_t *key, uint32_t klen, uint8_t *digest, DigestState *s);
static DigestState*sslmac_sha1(uint8_t *p, uint32_t len, uint8_t *key, uint32_t klen, uint8_t *digest, DigestState *s);
static DigestState*nomac(uint8_t *p, uint32_t len, uint8_t *key, uint32_t klen, uint8_t *digest, DigestState *s);
static void	sslPackMac(Secret *sec, uint8_t *mackey, uint8_t *seq, uint8_t *header, uint8_t *body, int len, uint8_t *mac);
static void	tlsPackMac(Secret *sec, uint8_t *mackey, uint8_t *seq, uint8_t *header, uint8_t *body, int len, uint8_t *mac);
...
//static uint32_t	get32(uint8_t *p);	// not used
...
static int	aesenc(Secret *sec, uint8_t *buf, int n);
static int	aesdec(Secret *sec, uint8_t *buf, int n);
...
static int
tlsgen(Chan *c, char* _1, Dirtab * _2, int _3, int s, Dir *dp)
{
	Qid q;
	TlsRec *tr;
	char *nm;
...
static Chan*
tlsattach(Chan *c, Chan *ac, char *spec, int flags)
{
	c = devattach('a', spec);
...
static long
tlsstat(Chan *c, uint8_t *db, long n)
{
...
static long
tlswstat(Chan *c, uint8_t *dp, long n)
{
	Dir *d;
	TlsRec *tr;
	int rv;
...
tlsrecread(TlsRec *tr)
{
	OneWay *volatile in;
	Block *volatile b;
	uint8_t *p, seq[8], header[RecHdrLen], hmac[MaxMacLen];
...
static Block*
tlsbread(Chan *c, long n, int64_t offset)
{
	int ty;
	Block *b;
	TlsRec *volatile tr;
...
static long
tlsread(Chan *c, void *a, long n, int64_t off)
{
	Block *volatile b;
	Block *nb;
	uint8_t *va;
...
static void
initclearmac(Hashalg * _1, int _2, Secret *s, uint8_t * _3)
{
	s->maclen = 0;
	s->mac = nomac;
}
...
static void
initRC4key(Encalg *ea, Secret *s, uint8_t *p, uint8_t * _1)
{
...
static void
initDES3key(Encalg * _1, Secret *s, uint8_t *p, uint8_t *iv)
{
...
static void
initAESkey(Encalg *ea, Secret *s, uint8_t *p, uint8_t *iv)
{
...
static void
initclearenc(Encalg * _1, Secret *s, uint8_t * _2, uint8_t * _3)
{
	s->enc = noenc;
	s->dec = noenc;
	s->block = 0;
...
static long
tlswrite(Chan *c, void *a, long n, int64_t off)
{
	Encalg *ea;
	Hashalg *ha;
	TlsRec *volatile tr;
...
static int
noenc(Secret * _1, uint8_t * _2, int n)
{
	return n;
}
...
static int
aesenc(Secret *sec, uint8_t *buf, int n)
{
	n = blockpad(buf, n, 16);
...
static int
aesdec(Secret *sec, uint8_t *buf, int n)
{
...
static DigestState*
nomac(uint8_t * _1, uint32_t _2, uint8_t * _3, uint32_t _4, uint8_t * _5, DigestState * _6)
{
	return nil;
}
 ...
 */
static DigestState*
sslmac_x(uint8_t *p, uint32_t len, uint8_t *key, uint32_t klen, uint8_t *digest, DigestState *s,
	DigestState*(*x)(uint8_t*, uint32_t, uint8_t*, DigestState*), int xlen, int padlen)
{
	int i;
...
static DigestState*
sslmac_sha1(uint8_t *p, uint32_t len, uint8_t *key, uint32_t klen, uint8_t *digest, DigestState *s)
{
	return sslmac_x(p, len, key, klen, digest, s, sha1, SHA1dlen, 40);
}

static DigestState*
sslmac_md5(uint8_t *p, uint32_t len, uint8_t *key, uint32_t klen, uint8_t *digest, DigestState *s)
{
	return sslmac_x(p, len, key, klen, digest, s, md5, MD5dlen, 48);
}

static void
sslPackMac(Secret *sec, uint8_t *mackey, uint8_t *seq, uint8_t *header, uint8_t *body, int len, uint8_t *mac)
{
	DigestState *s;
	uint8_t buf[11];
...
static void
tlsPackMac(Secret *sec, uint8_t *mackey, uint8_t *seq, uint8_t *header, uint8_t *body, int len, uint8_t *mac)
{
	DigestState *s;
	uint8_t buf[13];
