...
static int
usbgen(Chan *c, char * _, Dirtab* __, int ___, int s, Dir *dp)
{
	Qid q;
	Dirtab *dir;
	int perm;
...
static Chan*
usbattach(Chan *c, Chan *ac, char *spec, int flags)
{
...
static long
usbstat(Chan *c, uint8_t *db, long n)
{
	return devstat(c, db, n, nil, 0, usbgen);
}

/*
 * microseconds for the given transfer, for bandwidth allocation.
...
static long
ctlread(Chan *c, void *a, long n, int64_t offset)
{
	int q;
	char *s;
	char *us;
...
static long
usbread(Chan *c, void *a, long n, int64_t offset)
{
	int q;
	Ep *ep;
	int nr;
...
static long
usbwrite(Chan *c, void *a, long n, int64_t off)
{
	int nr, q;
	Ep *ep;
